function []=simulationCountMotherUponTime(filepath,bud,agg)
...
massD=zeros(n-1,1);
ratioCC=zeros(n-1,1);
meanMassM=zeros(n-1,1);
meanMassD=zeros(n-1,1);

tim=0;
for i=t(2:end)
    tim=tim+1;
    filename=strcat(filepath,'/',num2str(i),'.txt')
    [aggM,aggD,nM,nD]=simulationCountMother(filename);
    if bud
        Rbud=bud;
    ...
    nM_(tim,1)=length(aggM(:,1));
    nD_(tim,1)=length(aggD(:,1));
    massM(tim,1)=sum(aggM(:,6));
    massD(tim,1)=sum(aggD(:,6));
    meanMassM(tim,1)=mean(aggM(:,6));
    meanMassD(tim,1)=mean(aggD(:,6));
...
%ylabel('ratio of mass','Fontsize',12);
xlim([0 5400/60]);
ylim([0 2]);
hold off;

figure;
plot(t(2:end)./60,nM_/3758);
hold on;
plot(t(2:end)./60,nD_/32,'r-');
legend('mother','daugther');
hold off

figure;
plot(t(2:end)./60,meanMassM);
hold on;
plot(t(2:end)./60,meanMassD,'r-');
legend('mother','daughter');
hold off;
