function multiffToMatlabProject(folderName,binning,stacks,ID)
...
for i=1:channels
    ...
    %extracting size
    FileTif=[folderName,'/',nam,'_t001xy01z',num2str(midstacks),'.tif'];
    InfoImage=imfinfo(FileTif);
    mImage=InfoImage(1).Width;
    nImage=InfoImage(1).Height;
    %NumberImages=length(InfoImage);

    timeLapse.list(i).videoResolution(1)=nImage;
    timeLapse.list(i).videoResolution(2)=mImage;
...
nzerfrRead=3;%length(num2str(nframes));
nzerposRead=2%length(num2str(npos));
...
for pos=1:npos
    ...
    for ch=1:channels
        ...
        for frame=1:nframes
            ...
            if stacks(ch)==1
                %extracting multi tiff image
                FileTif=[folderName,'/',nam,'_t',framenumberRead,'xy',posnumber,'z',num2str(1),'.tif'];
                %%im=zeros(nImage,mImage,'uint16');
                im=imread(FileTif,'Index',ch);
        
            else    
                A=zeros(timeLapse.list(ch).videoResolution(1),timeLapse.list(ch).videoResolution(1),nstacks);
                for st=1:stacks(ch)
                    FileTif=[folderName,'/',nam,'_t',framenumberRead,'xy',posnumber,'z',num2str(st),'.tif'];
                    %image=zeros(nImage,mImage,'uint16');
                    image=imread(FileTif,'Index',ch);
                    A(:,:,st)=image;
                end
                clear im;
                im(:,:)=max(A,[],3);
