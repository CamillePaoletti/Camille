...
timeLapse.interval=inter; % to be determined later
timeLapse.sequencer=[];
timeLapse.seqFrame=[];
...
%timing=[]; 
intens=zeros(meta.channels,2); count=0;
    ...
    colorMaps = cell(numImages);
     ...
     for i = 1:numImages
        ...
        if numImages > 1
            ...
            sizeC = r.getSizeC();
            if sizeC > 0 %sizeC=1 for single channel timelapse
                qci=zct(2) + 1;
...
%timeLapse.interval=timing; %in seconds
