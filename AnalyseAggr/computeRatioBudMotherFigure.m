function [Data,areaMean,ratioMean,CcMean,meanLen]=computeRatioBudMotherFigure(path,file,position,hsFrame,lastFrame,meanLen)
...
for l=position
    ...
    if a==0
        ...
        [Cc,ratio,area,coupleNum]=computeRatioBudMother3(hsFrame,lastFrame,meanLen,0);
...
eventsName={'Right After HS','First Division','Next Budding'};
...
displayStatRatio(ratioMean,eventsLen);
...
hold off;
set(gca,'xtick',1:2, 'xticklabel',{'Mothers','Buds'}); 
...
legend(eventsName);
xlabel('Area');
ylabel('Bud to mother ratio of fluo');
title('Ratio of fluo in function of area');
hold off;
...
hold on;
xlabel('Area');
ylabel('Concentration of fluo');
title('Concentration of fluo in function of area');
hold off;
...
function displayStatRatio(ratio,eventsLen)
