function computeFociNew_aging(displayImage,l,cell2f)
%Camille Paoletti from Gilles Charvin - 11/2014
%link foci and cell they belong to in frame l, compute fluo in foci and
%cell ONLY for segmented cells passed in argument

global segmentation;

fprintf(['Processing linking frame ' num2str(l) ' \n']);

img=displayImage;

%  figure, imshow(img,[]);

cellnum=zeros(1,length(cell2f));
for j=1:length(cell2f)
    temp=find([segmentation.cells1(l,:).n]==cell2f(j));
    if isempty(temp)
        cellnum(1,j)=0;
    else
        cellnum(1,j)=temp;
...
for j=cellnum
    ...
    elseif segmentation.cells1(l,j).n~=0
        ...
        segmentation.cells1(l,j).fluoMax=sum(foci(1,:));%sum of area of foci in pix
