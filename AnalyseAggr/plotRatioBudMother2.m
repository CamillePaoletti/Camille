function [h_figure,ratioMean]=plotRatioBudMother(meanLen,Data,display,Volume)
...
if eventsLen>2
    bh=boxplot([transpose(ratioMean(1,:)),transpose(ratioMean(2,:)),transpose(ratioMean(3,:))],'notch','on');
    
    for j=1:size(bh(:),1) % <- # graphics handles/x
     set(bh(j),'linewidth',2);
    end
%set(bh(7,1),'Visible','off')
%set(bh(7,2),'Visible','off')
...
delete(bh(7,2))
    ...
    bh=boxplot([transpose(ratioMean(1,:)),transpose(ratioMean(2,:))],'notch','on');
   % set(bh(7,1),'Visible','off');
   % set(bh(7,2),'Visible','off');
...
bh=boxplot([transpose(CcMean(1,:,i)),transpose(CcMean(2,:,i))],'notch','on');
...
% Plot ratio at division as a function of bud size upon heat shock

pix1=find(~isnan(ratioMean(2,:)));
pix2=find(~isnan(Volume(2,:)));
pix=intersect(pix1,pix2);
rat=ratioMean(2,pix);
vol=Volume(2,pix);


figure, plot(vol,rat,'Marker','.','LineStyle','none','MarkerSize',8,'Color','k');
ylim([0 4]); xlim([0 2500]);
xlabel('Bud size upon HS (pix.)');
ylabel('Ratio [B]/[M] at div');
set(gca,'FontSize',20);
h_figure(end+1)=gca;
