...
for i=1:n1
    for j=1:n2
        if find(OD{i,j}==inf)
            ...
            stdeOD(i,j)=std(A(numel))/length(A(numel))+0.005;
        else
            meanOD(i,j)=mean(OD{i,j});
            stdeOD(i,j)=std(OD{i,j})/sqrt(length(OD{i,j}))+0.005;
        end
    end
end
...
hold on;
for k=[1 2 4 5]
    errorbar(time*60,log(meanOD(k,:)),(stdeOD(k,:).^2),Colors{k});   
    %plot(time*60,log(meanOD(k,:)),Colors{k});
end
%legend('25°C','30°C','32°C','35°C','37°C');
legend('25°C','30°C','35°C','37°C');
xlabel('time (min)');
...
for i=[1 2 4 5]
    for k=l%1:l
        ...
        str1=['fit ',num2str(i),': a=',num2str(p2{i,k}(1)),' doubling time=',num2str(log(2)/p2{i,k}(1))];
